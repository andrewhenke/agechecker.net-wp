	...
	class WC_Integration_AgeChecker_Integration extends WC_Integration {
					...
					<fieldset>
						...
						<p>
							<select id="<?php echo $data['list']; ?>Input" name="<?php echo $data['list']; ?>Input">
								<option value="<?php echo $default; ?>" <?php echo ($this->get_option($key) === $default ? "selected" : ""); ?>>None</option>
								<?php
									foreach ($array as $k => $c) {
										echo '<option value="' . $k . '" '. ($this->get_option($key) === $k ? "selected" : "") .'>' . $c . '</option>';
									}
								?>
							</select>
						</p>
		...
		public function generate_settingshider_html($key, $data) {
			...
			<script>
				...
				"#woocommerce_integration-agechecker_script_location", "#woocommerce_integration-agechecker_tag_order", "#woocommerce_integration-agechecker_page", "#woocommerce_integration-agechecker_form_trigger_name",
				...
				"#order_status_disabledInput", "#order_status_blockedInput"];
...
a.onerror=function(a){w.location.href="https://agechecker.net/loaderror";};h.insertBefore(a,h.firstChild);
		...
		function print_script_afterpayment($order_id) {
			...
			data: {
				...
				address: "<?php echo $order->get_billing_address_1(); ?>",
